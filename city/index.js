...
.aggregate()
.group({
    _id :{department: "$department", population:{$sum:"$population"}}
}).sort({_id:1}).then((response) => console.log(response));


/*citiesModel
.aggregate()
.group({
    _id :"$department", populationMoyenne:{$avg:"$population"}
}).sort({_id:1}).then((response) => console.log(response));*/
...
.group({
    _id :"$department", populationMoyenne:{$avg:"$population"}
}).sort({populationMoyenne: 1}).then((response) => console.log(response));
